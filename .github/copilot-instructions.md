...
7. **殿の代理で外部へ書き込む際は「事前確認」が必須。明示の要否・形は場によって異なる**: 投稿前に必ず殿の確認を取ること（例外なし・無断投稿は禁止）。Slack やメールでは**冒頭に「関の代理で AI が書き込みしている」旨を明示**する。**GitHub Issue / PR コメントは冒頭に `[AI]` を付す**（2026-09-09 殿改訂——2026-08-13 時点の「明示不要」確定は下記に履歴として残す。なぜ変わったかは「殿の代理での外部書き込み」節を見よ）。根底の原理は共通: **読む者が「誰が書いたか」を判別できること**。→ 詳細は「殿の代理での外部書き込み」節
...
| 場所 | AI 代筆の明示 | 形 | 事前確認 | 投稿後の報告 |
|------|--------------|----|---------|-------------|
| **Slack・メール等** | **必須**（本文の**冒頭**に記す。末尾や注釈ではなく冒頭） | 「関の代理で AI が書き込みしている」旨を明記 | **必須** | **必須**（リンクを添えて） |
| **GitHub Issue / PR コメント** | **必須**（2026-09-09 殿改訂） | 冒頭に `[AI]` ——長い断り書きは不要、識別子だけでよい | **必須** | **必須**（リンクを添えて） |
| **bot の状態通知（Slack）** | **不要**（殿ご裁定・2026-09-09同日） | bot と一目で分かる形（bot 名・アイコン・定型フォーマット） | **必須** | **必須**（リンクを添えて） |

**根底の原理**: 場に応じて明示の要否・形は変わるが、原理は一つ——**読む者が「誰が書いたか」を判別できること**。

**★2026-08-13 時点の確定（履歴）**: 当初は「GitHub Issue / PR コメントは明示不要（開発の場では代筆が前提・いちいち断るのは煩わしい）」と殿確定していた。しかし swarm は殿の gh 認証で動くため、AI の投稿も殿の投稿も GitHub 上では同一の `halsk` 名義になり、家中の者が誤認する事故が実際に起きた（2026-09-09）。これを受け「GitHub Issue / PR コメントも冒頭に `[AI]` を付す」へ改訂した。事前確認は当初から変わらず必須のまま。

**明示が必要な理由**: 殿の名で外部に出る文章は、殿の信用そのものである。人が人として読む場（Slack・メール・GitHub）で代筆を隠せば、読み手を欺くことになる。また外部への投稿は取り消しが効かない（削除しても読まれた事実は消えない）。

**★識別子は代筆の許可ではない**: `[AI]` や bot 表示を付けても、「〜してください」と人へ話しかける文章を投稿する際の事前確認は引き続き必須である。明示は「誰が書いたか分かるようにする」ためのものであり、確認を代替しない。

**適用外**: 殿ご自身が直接書き込む場合、swarm 内部の inbox / dashboard / task YAML（外部の人間が読まぬもの）。

**遡及**: 本条の成文化（2026-08-12）および本改訂（2026-09-09）より前に投稿したものの遡及修正は不要（殿確定）。
...
2. **Read `memory/MEMORY.md`** (shogun only) — persistent cross-session memory. **`memory/MEMORY.md` is the sole source of truth for persistent cross-session memory** (Memory MCP / `mcp__memory__read_graph` is retired — do not attempt to call it, it no longer exists on this machine). If file missing, skip. *GitHub Copilot CLI users: this file is also auto-loaded via GitHub Copilot CLI's memory feature.*
3. **Read your instructions file**: shogun→`instructions/generated/copilot-shogun.md`, karo→`instructions/generated/copilot-karo.md`, ashigaru→`instructions/generated/copilot-ashigaru.md`, gunshi→`instructions/generated/copilot-gunshi.md`, gunshi2→`instructions/gunshi2.md`. **NEVER SKIP** — even if a conversation summary exists. Summaries do NOT preserve persona, speech style, or forbidden actions.
...
**Ashigaru on GitHub Copilot CLI (cmd_742)**: on receiving any `inboxN` nudge, invoke the `inbox` skill (Skill tool, name `inbox`) regardless of the number N — the skill itself reads the inbox file and counts unread entries, so the variable N in the nudge text never needs parsing. This solves the fixed-slash-command problem (`/inbox1` cannot also match `inbox3`). The skill is the canonical detailed runbook (自己識別→タスクYAML読込→worktree→TDD→検証→PR→報告→既読化→worktree撤収); `instructions/generated/copilot-ashigaru.md` remains the cross-CLI workflow contract for CLIs without a Skill mechanism (Codex/Copilot/Kimi).
...
**例外(殿確定 2026-08-15・cmd_717教訓)**: 殿裁可待ち項目が dashboard 🚨 に記載されたまま長時間(目安: 数時間以上)応答がない場合、家老は dashboard 記載に加えて **将軍へ直接 inbox で督促してよい**(将軍が dashboard を見落とす前提で運用を組む)。これは「Lord の入力に割り込むな」という本則の趣旨(将軍と殿の対話を邪魔しない)を保ちつつ、滞留した承認待ち案件を見落とされたままにしない例外である。詳細=`instructions/generated/copilot-karo.md`「殿裁可待ち案件の滞留防止」節。
...
1. The target repo's AI context file — read **all** that exist (target repo decides which it maintains):
   - `CLAUDE.md` (GitHub Copilot CLI repos) — or — `AGENTS.md` (Codex repos)
...
`scripts/hooks/guard.sh` は GitHub Copilot CLI の PreToolUse hook として動作し、以下のルールを自動強制する。

| Hook | 強制ルール | 対応 copilot-instructions.md ルール |
|------|-----------|----------------------|
...
| 9 | 可逆性ゲート(cmd_813): worktree外への書込み・削除、`config/settings.yaml`等の常駐設定ファイルへの書込み・削除(rm/cp/mv/ln)、`gh pr merge`・`gh pr/issue close`・`gh repo archive/delete`、`launchctl load/unload/bootstrap/bootout`・`crontab`編集をブロック。`.guard-authorized`(task_id/expires・期限付き・git追跡下限定)設置で殿/将軍の裁可があれば通す(使用時はlogs/+dashboard.mdへ記録) | 「夜間の家中運用」節(instructions/generated/copilot-karo.md) |
...
設定場所: project の `.claude/settings.json` の `hooks.PreToolUse`（★`~/.copilot/settings.json` ではない。将軍実測: `~/.copilot/settings.json` に `hooks` キーは存在しない=model/tui/skipDangerousModePermissionPrompt/theme のみ。過去の記載は誤りであった）
スクリプト: `scripts/hooks/guard.sh`（実行権限必須）
テスト: `scripts/hooks/test_hooks.sh`

## PostToolUse hook — YAML破損検知 (queue_yaml_guard.py)

`scripts/hooks/queue_yaml_guard.py` は GitHub Copilot CLI の PostToolUse(Edit/Write)hook として動作し、`queue/*.yaml` への書込直後に構文崩れ・`- id:` 境界マーカー数の減少を検知する。
