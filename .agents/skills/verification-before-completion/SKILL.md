...
### Step 4.6: 原稿と成果物の鮮度突き合わせ — 原稿だけ直して満足するな(cmd_730 教訓)

原稿(.md 等)を編集しても、そこから派生する成果物(.pdf/.html/.pptx 等)の
再生成を忘れる事故がある。「原稿は直したが配布物は古いまま」を検出する。

- 原稿と成果物の**両方の更新時刻**を `ls -la` 等で突き合わせる
- 成果物が原稿より**新しい(または同時刻)**であることを実測で示す
- 古ければ再生成し、再生成後は Step 4.5 の要領で全ページ/全体を実描画確認する
- ★「さっき原稿を直したから大丈夫」は証拠にならない — 派生成果物ごとに個別に鮮度確認せよ
...
| 「原稿を直したから成果物も直っているはず」 | 派生生成物(PDF/HTML等)は都度再生成が要る。鮮度は `ls -la` 等の実測で確認せよ |
