...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "エイリアス:"
            echo "  csst  → cd /Users/hal/tools/multi-agent-shogun && ./shutsujin_departure.sh"
...
" 2>/dev/null
    ...
    if [ "$KESSEN_MODE" = true ]; then
        ...
        # 平時の陣: 3階層 --effort 設定（Q3確定方針）
        # Top:    ashigaru7       → Opus + --effort max
        # Boost:  ashigaru6       → Sonnet + --effort max
        # Standard: ashigaru1-5  → Sonnet 標準（コスト最適化）
        for i in $(seq 1 "$_ASHIGARU_COUNT"); do
            p=$((PANE_BASE + i))
            _ashi_cli_type="claude"
            if [ "$i" -eq 7 ]; then
