...
- **夜間の新規着手は軽い作業に限る**: 棚卸し・調査・既存PRの追い込み・status整合・report整理・小さな試験・ドキュメント是正。**重い作業(新規実装・大規模変換・E2E全走・複数リポ横断・実ブラウザ検証)を夜間に新規着手させるな**。既に走っている重い作業は中断させず継続させてよい。
- 殿への ntfy は夜間は引き続き発火させぬ(現状維持)。

### 夜間dispatch前チェック(subtask_781_night_limit_gap・機械的判定)

上の2方針(3〜4名・軽い作業)は文章ガイドのままでは強制力が無かった(9/7深夜に8〜9名全員が稼働し、cmd_779実装・cmd_780大規模変換などの重い作業が23〜01時に新規着手された実測あり)。夜間にタスクを新規割当する**直前**、毎回この2つを判定してから発注する。

1. **夜間判定**: `h=$(date +%H); [ "$h" -ge 22 ] || [ "$h" -lt 8 ]` が真なら夜間。偽なら本節は適用外。
2. **同時稼働数の上限=4名**(目安3名):
...
3. **重さ判定は task YAML の既存フィールドで機械的に行う**: これから割り当てるtaskが `bloom_level: L4` 以上、または `risk_flag: true` なら夜間の新規着手対象から外し、朝(8時以降)へ回す。L1〜L3 かつ risk_flag: false のみ夜間発注可。bloom_level は writing-task-yaml skill の必須項目ゆえ全taskに存在する——「軽いかどうか」を家老の感覚で判じない。

死者確認スイッチの夜間通知(「軽い作業のみ回せ」付記)を受けて動く場合も、このチェックを経てから発注する。判定はコマンド実行結果で行い、記録(何名稼働中・何を見送ったか)を朝の報告に含める。
