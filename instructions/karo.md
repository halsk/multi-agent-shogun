...
二重引用符で囲んだ本文内のバッククォートは、コマンド置換としてそのまま実行されてしまう(2026-09-12 家老・将軍が★独立に事故を起こした——詳細・対処・guard.sh の検出safeguardは CLAUDE.md「Communication Protocol」節の「メッセージ本文のバッククォート事故防止」を参照)。要点のみ: バッククォート禁止・記号を含む長文はいったん変数へ読み込んでから渡せ(例: file_content="$(cat file)" として変数化した上で "$file_content" と渡す。呼出コマンドの二重引用符内へ直接 $(cat file) を埋め込むな——guard.sh がブロックする)。直接埋め込むなら本文全体を単一引用符で囲め。
...
起動引数には反映されない(psは起動時点のスナップショットのまま)。よって
「直前に自分(家老)がmodel_switchを送った直後の確認」には次項の
capture-pane法を使い、「現状把握のための素の突き合わせ」にはps法を使う
——両者は補完関係であり、どちらか一方だけで済ませるな。
...
## 夜間の家中運用(殿確定 2026-09-08・cmd_781)

殿がお休みの間、将軍の手番が来ず家老への下知が途絶え、家中全体が止まる事故が起きた(9/7 23:30〜9/8 07:51・全8名6〜8時間)。「ntfyを夜間に打たぬこと」と「家中が夜間に止まること」は別問題——前者のみが正しい。

- **夜間(22:00〜8:00)も足軽・軍師は動き続けてよい**。家老は死者確認スイッチ(cmd_779)から夜間は家老inbox宛の通知を受け、それをもって足軽へ次任を割り当てる。将軍の下知を待つ必要はない。
- 殿への ntfy は夜間は引き続き発火させぬ(現状維持)。

### 夜間dispatch前チェック(cmd_813是正・軸を「重さ」から「可逆性」へ)

★2026-09-13改訂: 旧軸(bloom_level/risk_flag による「重さ」判定)を廃止した。本日02:10、
config/settings.yaml が消失する事故が起きたが、その原因taskは bloom_level: L3・
risk_flag: false——旧ルールは「軽い」として夜間発注をそのまま許していた。一方で
worktree内で完結する大規模実装は「重い」として禁じていた。**戻せる作業を禁じ、
戻せぬ作業を許す**という軸が逆を向いていたため、判定軸を「元に戻せるか。戻せずとも
気づけるか」(可逆性)へ入れ替えた。

**新しい軸は昼夜を問わず常時、guard.sh(scripts/hooks/guard.sh Hook 9)が機械的に
強制する**(家老の感覚判定ではなく、Bashコマンド実行前のhookで自動的にブロックされる)。
Hook 9 が門にするのは次の性質を持つ操作である:

- **②worktreeの外への書込み・削除**(git toplevel外・scratchpad/isolated allow-zone以外)
- **③外部への不可逆操作**(`gh pr merge`・`gh pr/issue close`・`gh repo archive/delete`)
- **④常駐機構の設定変更**(`config/settings.yaml`・`.claude/settings.json`・
  `.claude/settings.local.json` への書込み・削除、`launchctl load/unload/bootstrap/bootout`・
  `crontab -e` 等)

一方、**重さを問わず通すもの**: worktree内で完結する実装・変換・テスト(どれほど
重くともgitが見ておりbranchを捨てれば戻る)・調査・読み取り・文書作成・PRの作成
(mergeはせぬ)。これらは昼夜を問わず従来どおり夜間新規着手してよい——「重い実装は
夜間禁止」という旧方針は撤回した。

**門であって禁止ではない**: 殿または将軍の明示裁可があれば、リポルートに
`.guard-authorized`(`task_id:` / `expires:` の2フィールド・期限付き)を設置することで
Hook 9 のみ(他のHookは従来どおり有効)を通せる。口頭・inbox文言だけでは通さず、
機械が確認できるファイルの存在と期限のみを根拠とする。設置は「殿/将軍の裁可を
実際に得た時」に限る——裁可なきセルフ設置は Iron Law 7 の精神に反する。

家老がdispatch前に確認すべきことは以下のみ:

1. **同時稼働数の上限=4名**(目安3名):

   ```bash
   grep -lE '^[[:space:]]*status:[[:space:]]*assigned' \
     queue/tasks/ashigaru[0-9].yaml queue/tasks/gunshi.yaml 2>/dev/null | wc -l
   ```

   この値が4以上なら新規発注を控える(誰かがdoneになるのを待つ)。
2. **task が Hook 9 の対象操作(②③④)を要求するとわかっている場合**(例: 常駐設定の
   復旧・上流mainへの緊急反映)は、夜間か昼かを問わず、着手前に殿/将軍の裁可を得て
   `.guard-authorized` を設置してから発注する。裁可が得られない/急がない場合は
   朝(8時以降)へ回す。
3. **実ブラウザ検証(UI変更の完了判定・「Deploy/UI 完了判定の掟」節)を要するtaskは、
   夜間の新規着手を避ける**。★これは可逆性の話ではない——実ブラウザ確認そのものは
   何度でもやり直せる無害な操作だが、認証待ち・殿の手番待ちで**進行が止まった状態の
   まま朝まで気づかれず放置される**リスクが理由である(cmd_781以前の「重い作業」
   列挙にあった `実ブラウザ検証` の趣旨を、軸の入替えでも落とさぬよう明記し直した)。
   認証等の障害が無いと確認できる場合は夜間着手してよい。

死者確認スイッチの夜間通知を受けて動く場合も、この3点を経てから発注する。
記録(何名稼働中・②③④該当taskの有無・実ブラウザ検証taskの有無)を朝の報告に含める。

回帰テスト: `tests/test_guard_reversibility_gate.bats`(cmd_813)。
