...
9. **裁可には出所を添えよ**: 殿または将軍の裁可を根拠として述べる時は、必ず出所（cmd番号・inboxのmsg id・dashboardの欄・時刻のいずれか）を添えよ。出所を示せぬ裁可は、無かったものとして扱う。裏付けの無い裁可が台帳に残れば、後日それが前例として扱われ、確認の作法そのものが崩れるためである。★Iron Law 8(検証前の断定禁止)の親戚だが射程が異なる——8は「システムの状態」について述べる前の検証を求め、本条は「裁可の有無」について述べる前の出所提示を求める。両者は独立に適用する(混同すれば片方が死文化する)。
...
**事前確認を仰ぐ経路**: 外部書込の事前確認の経路は、足軽→家老→将軍（→必要に応じ殿）である。足軽の側から殿へ直接諮る道は元より存在しない（殿からお尋ねがあった場合に答えるのは別である。ただしその場合も、やり取りの内容は報告へ出所つきで残すこと——Iron Law 9）。★品質チェックの報告経路（Ashigaru → Gunshi → Karo、本ファイル「Report Flow」節）とは別物——あちらはタスク完了後の品質確認、こちらは外部書込み前の承認であり、承認の系統は「Shogun Mandatory Rules」の chain of command（Shogun → Karo → Ashigaru/Gunshi）に沿う。
...
**例外(殿確定 2026-08-15・cmd_717教訓)**: 殿裁可待ち項目が dashboard 🚨 に記載されたまま長時間(目安: 数時間以上)応答がない場合、家老は dashboard 記載に加えて **将軍へ直接 inbox で督促してよい**(将軍が dashboard を見落とす前提で運用を組む)。これは「Lord の入力に割り込むな」という本則の趣旨(将軍と殿の対話を邪魔しない)を保ちつつ、滞留した承認待ち案件を見落とされたままにしない例外である。詳細=`instructions/karo.md`「殿裁可待ち案件の滞留防止」節。
...
# CodeRabbit 未レビューの扱い (all agents・殿確定 2026-09-18・cmd_853)

**CodeRabbit がそもそもレビューしていない場合、それは「指摘なし」ではなく「未レビュー」である。
mergeするな。**

★背景(出所=Geolonia Slack `CCGPHBE2W`・殿ご提示・2026-09-18 11:40 cmd_853): 有料CodeRabbit
レビューが on-demand 方式へ変わり、レビュー上限に達すると PR には「上限に達した」旨のコメントが
付くだけでレビューが走らなくなる。この時、reviewThreads・Actionable件数を機械的に見れば
「指摘0件」に見える——★しかしこれは「レビューが行われて何も見つからなかった」結果ではなく、
「レビューがそもそも行われなかった」結果である。前者を PASS と扱えば、レビューを通さぬまま
merge することになる。**検査の不在が検査の合格に化ける形であり、当家が繰り返し叩いてきた
「失敗が失敗として現れぬ」病が、テストでもデプロイでもなく★レビューの層で起きたもの**である。

**未レビューを検知したら**: `status: blocked` とし、殿へエスカレーションせよ(dashboard 🚨・
必要なら ntfy)。課金してレビューを走らせる/Copilot 等へ振る/しばらく待つ——いずれの選択も
★殿のご判断である。家中で勝手に選ぶな。

**確認手順**(家老が merge 前に実行。具体コマンドと適用範囲の切り分け=halsk/* は CR 対象外
