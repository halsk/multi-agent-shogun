...
**事前確認を仰ぐ経路**: 外部書込の事前確認の経路は、足軽→家老→将軍（→必要に応じ殿）である。足軽が殿へ直接諮る道は元より存在しない。★品質チェックの報告経路（Ashigaru → Gunshi → Karo、本ファイル「Report Flow」節）とは別物——あちらはタスク完了後の品質確認、こちらは外部書込み前の承認であり、承認の系統は「Shogun Mandatory Rules」の chain of command（Shogun → Karo → Ashigaru/Gunshi）に沿う。
